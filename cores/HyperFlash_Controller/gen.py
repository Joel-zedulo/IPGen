...
try:
    ...
    print("Could not import HyperRAM from litehyperbus. Ensure it is installed.")
    sys.exit(1)
...
class HyperFlashSoC(SoCMini):
    def __init__(self, platform, core_config):
        ...
        # LiteHyperBus uses HyperRAM core class for the HyperBus hardware protocol block
        ...
        # Standard Flash memory-mapped region origin
...
def main():
    ...
    toolchain = cfg.get("toolchain", "vivado")
    ...
    soc = HyperFlashSoC(platform, cfg)

    builder = Builder(soc, output_dir="./build", compile_gateware=False, compile_software=False)

    core_name = os.environ.get("CORE_NAME", "hyperflash_controller")
